...
// IP / 单段等其他 → 拒绝
function classifyHost(hostname: string): Decision {
  const host = hostname.toLowerCase().replace(/\.$/, "");
  if (isIP(host)) return "deny";
  const labels = host.split(".").filter(Boolean);
  if (labels.length === 2) return "ask";
  if (labels.length >= 3) return "allow";
  return "deny";
}

// 从 curl 命令里解析出目标主机名（启发式，覆盖常见写法）
function extractHostname(command: string): string | null {
  // 优先匹配带协议的完整 URL
  ...
  for (const token of command.split(/\s+/)) {
    ...
    if (t.startsWith("-")) continue; // 跳过选项参数
    if (/^[\w-]+(\.[\w-]+)+$/.test(t)) {
...
async function runCurl(command: string): Promise<string> {
  try {
    ...
    return `curl 执行失败: ${(err as Error).message}`;
  }
}
...
server.tool(
  "execute_curl",
  "执行 curl 命令，并按域名层级放行 / 问询 / 拒绝。二级及以上子域名（如 xxx.example.com）直接执行；一级域名（如 example.com）需用户确认；IP 等其他情况一律拒绝。",
  {
